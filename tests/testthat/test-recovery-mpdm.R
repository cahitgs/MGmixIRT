test_that("MPDM estimator reaches the MLE and recovers stable parameters", {
  skip_on_cran()
  set.seed(42)
  a_true <- exp(rnorm(15, 0, 0.2))
  b_true <- seq(1.2, -1.2, length.out = 15)
  truth <- list(pi_nd = c(0.50, 0.65), omega = c(3, 4.5),
                kappa = c(0.5, 0.8), mu_nd = c(0, 0.5),
                rho = c(-0.05, -0.08), sigma = c(1, 0.95))
  sim <- sim_mgmixirt("mpdm", n = c(1500, 1500), I = 15, a = a_true,
                      b = b_true, pi_nd = truth$pi_nd,
                      omega = truth$omega, kappa = truth$kappa,
                      mu_nd = truth$mu_nd, rho = truth$rho,
                      sigma = truth$sigma, seed = 602)
  fit <- mgmixirt(sim$resp, sim$group, model = "mpdm", quadpts = 13,
                  starts = c(3, 10), maxit = 2000, tol = 1e-5,
                  seed = 11, verbose = FALSE)
  expect_true(fit$converged)
  expect_true(all(diff(fit$trace) > -1e-4))

  spec <- MGmixIRT:::mgm_spec("mpdm", 15L, 2L)
  quad <- MGmixIRT:::gh_quad(13)
  par_t <- list(a = a_true, b = b_true, mu_nd = truth$mu_nd,
                logsigma = log(truth$sigma), rho = truth$rho,
                logkappa = log(truth$kappa),
  ...
  expect_lt(co$groups$sigma[2], 1.15)
